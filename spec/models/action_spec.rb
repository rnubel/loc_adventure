...
describe Action do
  ...
  it "should change its status to effected when effected" do
    action = Action.new
    action.expects(:effectable?).returns(true)
    action.expects(:type).returns(stub(:transactions => []))
  ...
  describe Action::Type::DRAW do
    before(:each) do
      ...
      account2 = Factory.create(:account, :account_type => Account::Type::PRINCIPAL)
      @action.line_of_credit.expects('find_account').with(:customer).returns(account1)
      @action.line_of_credit.expects('find_account').with(:principal).returns(account2)
      @action.effect!
    end
    ...
    it "should create a transaction from customer to receivable when effected" do
      @action.transactions.all? { |tr|
        ...
        tr.credit_account.account_type.should == Account::Type::PRINCIPAL
